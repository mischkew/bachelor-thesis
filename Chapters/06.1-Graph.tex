...
Now that all lines are found we have to test if the lines actually go through both plates. \TODO{this is not correct, in future work we have to make sure that the plates actually touch!} In addition, this step retrieves the exact start and end points of the line segment that defines the intersection of both plates.\\
...
\TODO{Image explaining our assumption}
...
\TODO{Alternatives: why not use middle axis going through joints? Good-bad-evaluation}
\TODO{write in future work why this assumption is crap and how we will handle this}
...
Now that all lines are known we need to shorten the lines so that no other lines overlap with it. If this step is missing then the following step for creating joints will run in to problems because the joints overlap each other.\\
...
Dustin used an adjacency matrix for keeping track of neighboring plates. All the plates were kept in a plate array. Those indeces were corresponding to the rows and columns of the matrix. Each cell contained a set of plate adjacency objects. This made it possible that one plate could have more than one connection.
...
\TODO{Find out if there was another bad thing about it like not finding all lines possible...}
...
\TODO{Probably move angle calculation section here and replace it with how we do it now: clip and based on that know the angle}
