...
\note{when intro of WebGl here, than give a footnote, else
  put into introduction} We use the Web Graphics Library
...
\note{THE FRAMEWORK thingy sounds weird}
...
\note{maybe too much recursion in the following paragraphs...}
The \name{PlatenerPipeline} plugin is the main computation
unit. The \class{Plugin} defines multiple {\fabmethod}s.
...
the 2D-construction plans. \class{PipelineSteps} are
...
\note{fabmethod thingy is not clear...}
\note{pipeline executes steps...}
Each {\fabmethod} assembles a \class{Pipeline} from
\class{PipelineSteps}. We call the state of a data structure
taken at a given point in time a snapshot. To improve the
development and debugging experience, we not only pass the
results of the \class{PipelineStep} to the following step,
...
\class{Pipeline} processes the results of
\class{PipelineSteps} and takes snapshots of the data. Note,
that the \class{PipelineSteps} are merely computation units.
All visual output is done in the \name{NodeVisualizer}
plugin.

\note{pipeline runs pipeline steps... or maybe have this in
  the fabmethod section}


As the algorithms of different \class{PipelineSteps} work
with same data structures, we have to create copies of the
data. Otherwise a latter \class{PipelineStep} will alter the
data in the snapshot of a previous \class{PipelineStep}.
Thus, the \name{NodeVisualizer} plugin renders corrupted
visuals for the previous step. Figure~\ref{} \note{figure of
corrupted shapes vs noncorrupted shapes} shows how the
...
The \class{PipelineState} is an clonable data structure. A
...
the data by latter \class{PipelineSteps} do not modify
previous copies of the data. A snapshot is obtained by
cloning the \class{PipelineState} after the execution of a
\class{PipelineStep}.

\note{Subsection summary}
