...
as they do not refer to each other explicitly. The mediator takes away
that necessity \cite[p. ?]{gof}\note{fix citing, lookup page}.
...
visualization before releasing all the computed data.

These hooks are registered at the \emph{Dispatcher}, which is
responsible for dispatching the lifecycle events to the plugins
in a predefined order\myNotes{, see chapter ??}. Beside
\emph{PluginHooks}, a \emph{Plugin} can expose \emph{Protocols}
to interact with the system. \emph{Protocols} are described in
\myNotes{chapter ??}.
