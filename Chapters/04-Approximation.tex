...
\myNotes{TODO: reread und rewrite}
...
This hemisphere is separated into rings. Each ring covers the threshold angle in $z$-direction and is visualized in Figure \ref{fig:rings_FromSide} and Figure \ref{fig:rings_FromTop}. The correlating angle of a normal is proporional to its coordinates as demonstraded in Figure \ref{fig:ringAngle}:
...
The angle $\alpha$ is represented as a multiple of the threshold angle to get the actual ring index:
...
The maximum angle is 90° (normal $ = (0,0,1)$). Consequently the maximum ring index is calculated as follows:
...
Each ring is seperated into buckets which cover the threshold angle as shown in Figure \ref{fig:PrismCl-buckets}. Therefore the onto to xy-plane projected angle $\gamma$ needs to be calculated which differs from ring to ring. This 
